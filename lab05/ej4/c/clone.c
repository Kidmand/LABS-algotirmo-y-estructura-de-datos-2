...
int main(void) { 
    ...
    char *copy=NULL;
    copy = string_clone(original, strlen(original)/sizeof(char) - 1); // esta es cuando *original;
    // copy = string_clone(original, sizeof(original)/sizeof(char) - 1); //esta es cunado es original[];
    printf("Original: %s\n", original);
    copy[0] = 'A';
    copy[1] = ' ';
    copy[2] = 'l';
